...
### SUMAS PONDERADAS DE INGESTA DE SUBGRUPOS DE ALIMENTOS EN ENALIA ###
#######################################################################

# Suma de las medias, raiz cuadrada de los ^2 de los SEs

setwd("C:/Users/ALHE/OneDrive - Folkehelseinstituttet/Articles/Food groups_NHS/Data")
...
vars01<-c("verdura","fruta","legumbres","pan_cereales","pasta_arroz_patata","pescado","carne",
          "embutidos","lacteos","dulces","refrescos","huevos")
vars02<-c("Intake of vegetables (g/day)","Intake of fruits (g/day)",
          ...
          "Intake of fresh meat (g/day)","Intake of processed meat (g/day)",
          "Intake of dairy products (g/day)","Intake of sweets (g/day)",
          "Intake of soft drinks (g/day)","Intake of eggs (g/day)")
...
{
  namefile<-paste("./clean/",vars01[i],".csv",sep="")
  dat<-read.csv2(namefile,header=TRUE,sep=";",dec=".")[2:22]
  ...
  figure_all<-ggplot() +
    ...
    labs(
      ...
      x = "Year",
      y = vars02[i]) +
    scale_x_continuous(breaks = c(2003, 2006, 2009, 2011, 2014, 2017, 2020),
                       expand = c(0, 0.2)) +
  ...
  namefile<-paste("C:/Users/ALHE/OneDrive - Folkehelseinstituttet/Articles/Food groups_NHS/Outputs/",vars01[i],".jpg",sep="")
...
vars01<-c("zumos","aperitivos")
vars02<-c("Intake of fresh juices (g/day)","Intake of snacks (g/day)")
  ...
  
{
  ...
  mean_smooth <- lm(mean ~ as.numeric(year), data = df_01_04)
  lo_smooth <- lm(lo ~ as.numeric(year), data = df_01_04)
  hi_smooth <- lm(hi ~ as.numeric(year), data = df_01_04)
  years <- seq(min(as.numeric(df_men$year)), max(as.numeric(df_men$year)), by = 0.1)
  mean_pred <- predict(mean_smooth, newdata = data.frame(year = years))
  lo_pred <- predict(lo_smooth, newdata = data.frame(year = years))
  hi_pred <- predict(hi_smooth, newdata = data.frame(year = years))
  shade_df <- data.frame(year = years, mean = mean_pred, lo = lo_pred, hi = hi_pred)
  shade_df[1:80,2:4]<-NA
  shade_df[142:171,2:4]<-NA
  figure_01_04<-ggplot() +
    geom_ribbon(data = shade_df, aes(x = year, ymin = lo, ymax = hi), fill = "grey60", alpha = 0.5) +
    geom_smooth(data = shade_df, aes(x = year, y = mean), color = "black", se = FALSE) +
    theme_classic() +
    labs(
      title = "1-4 years",
      tag = "D",
  ...
  mean_smooth <- lm(mean ~ as.numeric(year), data = df_05_14)
  lo_smooth <- lm(lo ~ as.numeric(year), data = df_05_14)
  hi_smooth <- lm(hi ~ as.numeric(year), data = df_05_14)
  years <- seq(min(as.numeric(df_men$year)), max(as.numeric(df_men$year)), by = 0.1)
  mean_pred <- predict(mean_smooth, newdata = data.frame(year = years))
  lo_pred <- predict(lo_smooth, newdata = data.frame(year = years))
  hi_pred <- predict(hi_smooth, newdata = data.frame(year = years))
